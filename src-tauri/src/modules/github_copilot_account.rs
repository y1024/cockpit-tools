...
fn account_index_from_store() -> Result<GitHubCopilotAccountIndex, String> {
    ...
    let accounts = crate::modules::platform_account_store::list_accounts::<GitHubCopilotAccount>(
        ACCOUNT_STORE_PLATFORM,
    )?;
...
pub fn load_account(account_id: &str) -> Option<GitHubCopilotAccount> {
    ...
    } else if let Ok(Some(account)) = crate::modules::platform_account_store::load_account::<
