...
fn account_index_from_store() -> Result<QoderAccountIndex, String> {
    ...
    let accounts =
        crate::modules::platform_account_store::list_accounts::<QoderAccount>(ACCOUNT_STORE_PLATFORM)?;
...
pub fn load_account(account_id: &str) -> Option<QoderAccount> {
    ...
    } else if let Ok(Some(account)) = crate::modules::platform_account_store::load_account::<QoderAccount>(
        ACCOUNT_STORE_PLATFORM,
        account_id,
    ) {
        return Some(account);
